...
public class PartnerDatabaseRepository implements PartnerRepository {
  ...
  public Optional<Partner> partnerOfId(final PartnerId partnerId) {
    return this.partnerJpaRepository
        .findById(UUID.fromString(partnerId.value()))
        .map(it -> it.toPartner());
  }

  @Override
  public Optional<Partner> partnerOfCnpj(final Cnpj cnpj) {
    return this.partnerJpaRepository.findByCnpj(cnpj.value()).map(it -> it.toPartner());
  }

  @Override
  public Optional<Partner> partnerOfEmail(final Email email) {
    return this.partnerJpaRepository.findByEmail(email.value()).map(it -> it.toPartner());
  }
