...
public class EventDatabaseRepository implements EventRepository {
  ...
  public Optional<Event> eventOfId(EventId eventId) {
    return this.eventJpaRepository
        .findById(UUID.fromString(eventId.value()))
        .map(it -> it.toEvent());
  }

  @Override
  public Optional<Event> eventOfName(Name name) {
    return this.eventJpaRepository.findByName(name.value()).map(it -> it.toEvent());
  }

  @Override
  public Optional<Event> eventOfDate(String date) {
    return this.eventJpaRepository.findByDate(LocalDate.parse(date)).map(it -> it.toEvent());
  }
